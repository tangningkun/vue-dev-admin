...
export function useSiderEvent() {
    ...
    const { getMinWidthValue } = useMenuSetting();

    const getCollapsedWidth = computed(() => {
        return unref(brokenRef) ? 0 : unref(getMinWidthValue);
    });
...
export function useDragLine(siderRef: Ref<any>, dragBarRef: Ref<any>, mix = false) {
    const { getMinWidthValue, getCollapsed, setMenuSetting } = useMenuSetting();
    ...
    function handleMouseMove(ele: HTMLElement, wrap: HTMLElement, clientX: number) {
        document.onmousemove = function (innerE) {
            ...
            const maxT = 800;
            const minT = unref(getMinWidthValue);
            iT < 0 && (iT = 0);
            iT > maxT && (iT = maxT);
            iT < minT && (iT = minT);
    ...
    function removeMouseup(ele: any) {
        ...
        document.onmouseup = function () {
            ...
            if (!mix) {
                const miniWidth = unref(getMinWidthValue);
